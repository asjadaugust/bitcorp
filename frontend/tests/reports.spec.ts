...
test.describe('Reports & Analytics Page', () => {
  ...
  test('should display reports page with all sections', async ({ page }) => {
    ...
    await expect(page.getByText('Equipment valuation, performance analysis, and cost management')).toBeVisible();
  ...
  test('should display KPI metrics dashboard', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('h5:has-text("Key Performance Indicators (KPIs)")', { timeout: 10000 });
    ...
    const kpiCards = page.locator('.MuiCard-root').filter({ hasText: /Equipment Utilization Rate|Cost Savings|Timesheet Completion|Daily Report Compliance|Equipment Downtime|Average Equipment ROI/ });
  ...
  test('should display equipment performance table', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('h5:has-text("Equipment Performance Analysis")', { timeout: 10000 });
    
    // Check equipment performance section
    await expect(page.getByRole('heading', { name: /Equipment Performance Analysis/ })).toBeVisible();
    
    // Check table headers
    await expect(page.getByRole('columnheader', { name: 'Equipment' })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Type' })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Utilization Rate' })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Total Hours' })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: 'Cost per Hour' })).toBeVisible();
  ...
  test('should display available reports section', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('h5:has-text("Available Reports")', { timeout: 10000 });
    ...
    const reportCards = page.locator('.MuiCard-root').filter({ hasText: /Equipment Valuation Report|Performance Analytics Dashboard|Cost Analysis Report|Operator Performance Report|Maintenance Schedule Report/ });
    ...
    await expect(page.getByText('Performance Analytics Dashboard')).toBeVisible();
  ...
  test('should handle report generation', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('button:has-text("Generate Report")', { timeout: 10000 });
    
    // Click main generate report button
    const mainGenerateButton = page.getByRole('button', { name: 'Generate Report' }).first();
    ...
    await page.waitForSelector('button:has-text("Generate Report")', { timeout: 5000 });
    await expect(page.getByText('Generate Report')).toBeVisible();
  });

  test('should handle individual report generation', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('text=Equipment Valuation Report', { timeout: 10000 });
    
    // Find the first Generate button in available reports
    const firstGenerateButton = page.locator('.MuiCard-root').first().getByRole('button', { name: 'Generate' });
  ...
  test('should handle download buttons in available reports', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('text=Equipment Valuation Report', { timeout: 10000 });
    
    // Find the first Download button in available reports
    const firstDownloadButton = page.locator('.MuiCard-root').first().getByRole('button', { name: 'Download' });
  ...
  test('should handle back to dashboard navigation', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('button:has-text("Back to dashboard")', { timeout: 10000 });
    
    // Click back to dashboard button
    await page.click('button:has-text("Back to dashboard")');
    
    // Should navigate back to dashboard
    await page.waitForSelector('h1:has-text("Equipment Management Dashboard")', { timeout: 10000 });
    await expect(page.getByRole('heading', { name: /Equipment Management Dashboard/ })).toBeVisible();
  });
  ...
  test('should handle breadcrumb navigation', async ({ page }) => {
    ...
    const dashboardBreadcrumb = page.locator('nav[aria-label="breadcrumb"]').getByText('Dashboard');
  ...
  test('should display refresh data functionality', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('button:has-text("Refresh Data")', { timeout: 10000 });
  ...
  test('should display equipment status chips correctly', async ({ page }) => {
    ...
    const activeChips = page.locator('.MuiChip-root').filter({ hasText: 'active' });
    const maintenanceChips = page.locator('.MuiChip-root').filter({ hasText: 'maintenance' });
  ...
  test('should display utilization rate progress indicators', async ({ page }) => {
    await page.click('text=Reports & Analytics');
    await page.waitForSelector('h5:has-text("Equipment Performance Analysis")', { timeout: 10000 });
