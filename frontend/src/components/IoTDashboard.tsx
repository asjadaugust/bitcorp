...
const getMockDashboardData = (equipmentId: number) => ({
  ...
  alerts: [
    {
      ...
      message: 'Hydraulic oil temperature slightly elevated',
      action: 'Check hydraulic system cooling',
  ...
  iot_devices: [
    ...
    },
    {
      id: 2,
      device_id: 'BCT-002',
      type: 'Hydraulic Sensor',
      status: 'online' as const,
      last_communication: new Date().toISOString(),
    },
  ],
  predicted_savings: {
    annual_savings: 1250000,
    monthly_savings: 104167,
    breakdown: {
      breakdown_prevention: 500000,
      early_detection: 300000,
      fuel_efficiency: 250000,
      extended_lifespan: 200000,
    },
    roi_multiple: 348,
  },
});
...
export const IoTDashboard: React.FC<IoTDashboardProps> = ({
  ...
  const displayData = dashboardData || getMockDashboardData(equipmentId);
  ...
  if (error && !displayData) {
    return (
      <Alert severity="warning" sx={{ mb: 3 }}>
        {error?.message ||
          error?.toString() ||
          'IoT API not available - showing demo data'}
    ...
    <Box>
      ...
      {error && (
        <Alert severity="info" sx={{ mb: 3 }}>
          Demo Mode: IoT backend not connected - displaying sample data
        </Alert>
      )}
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Grid container spacing={3}>
            <Grid item xs={6} md={2}>
              <HealthScoreGauge
                score={displayData.health_score.overall}
                label="Overall"
                color="#4caf50"
              />
            </Grid>
            <Grid item xs={6} md={2}>
              <HealthScoreGauge
                score={displayData.health_score.engine}
                label="Engine"
              />
            </Grid>
            <Grid item xs={6} md={2}>
              <HealthScoreGauge
                score={displayData.health_score.hydraulic}
                label="Hydraulic"
              />
            </Grid>
            <Grid item xs={6} md={2}>
              <HealthScoreGauge
                score={displayData.health_score.transmission}
                label="Transmission"
              />
            </Grid>
            <Grid item xs={6} md={2}>
              <HealthScoreGauge
                score={displayData.health_score.electrical}
                label="Electrical"
              />
            </Grid>
            <Grid item xs={6} md={2}>
              <HealthScoreGauge
                score={displayData.health_score.structural}
                label="Structural"
              />
            </Grid>
          </Grid>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          <ROICard savings={displayData.predicted_savings} />
        </Grid>

        {/* Sensor Data */}
        {Object.entries(displayData.sensor_summary).map(
          ([sensorType, data]) => (
            ...
            <CardContent>
              ...
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {displayData.iot_devices.map((device) => (
