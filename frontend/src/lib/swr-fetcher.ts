...
export const authenticatedFetcher = async <T = unknown>(url: string, options: RequestInit = {}): Promise<T> => {
...
export const swrFetcher = <T = unknown>(url: string): Promise<T> => authenticatedFetcher<T>(url)

// Mutation helper for non-GET requests
export const mutationFetcher = async <T = unknown>(url: string, { method = 'POST', data }: { method?: string; data?: unknown } = {}): Promise<T> => {
...
export const getErrorMessage = (errorData: unknown, status?: number): string => {
...
export const authAPI = {
  ...
  login: async (data: LoginRequest): Promise<TokenResponse> => {
    return mutationFetcher<TokenResponse>('/auth/login', { method: 'POST', data })
  },
