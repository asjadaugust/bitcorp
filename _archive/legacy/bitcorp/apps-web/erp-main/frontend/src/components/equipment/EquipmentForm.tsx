  ...
  const onSubmit = async (data: EquipmentFormData) => {
    try {
      ...
      const cleanData = {
        ...
        year_manufactured:
          data.year_manufactured === ''
            ? undefined
            : Number(data.year_manufactured),
        purchase_cost:
          data.purchase_cost === '' ? undefined : Number(data.purchase_cost),
        current_value:
          data.current_value === '' ? undefined : Number(data.current_value),
        hourly_rate:
          data.hourly_rate === '' ? undefined : Number(data.hourly_rate),
        fuel_capacity:
          data.fuel_capacity === '' ? undefined : Number(data.fuel_capacity),
      ...
      if (isEditing && equipment) {
        ...
        delete (updateData as EquipmentUpdateRequest & { company_id?: number })
          .company_id; // Remove company_id from update data
        result = await updateEquipment({ id: equipment.id, data: updateData });
      } else {
        // Create new equipment
        const createData: EquipmentCreateRequest =
          cleanData as EquipmentCreateRequest;
