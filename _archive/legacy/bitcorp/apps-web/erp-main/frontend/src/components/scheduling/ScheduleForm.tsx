    ...
    >
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 3 }}>
          {error instanceof Error
            ? error.message
            : 'An error occurred while saving schedule'}
        </Alert>
      )}
      ...
      {hasConflicts && (
        <Alert severity="warning" icon={<WarningIcon />} sx={{ mb: 3 }}>
          ...
          {conflicts.map((conflict, index) => (
            <Chip
              key={index}
              label={`Conflict: ${format(
                new Date(conflict.overlap_start),
                'MMM dd, HH:mm'
              )} - ${format(new Date(conflict.overlap_end), 'HH:mm')}`}
              color="warning"
              size="small"
              sx={{ mt: 1, mr: 1 }}
            />
      ...
      {isEquipmentUnavailable && (
        <Alert severity="info" sx={{ mb: 3 }}>
          Equipment is currently unavailable on the selected date. Available
          periods: {availability?.available_periods.length || 0}
        </Alert>
      )}
              ...
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (date && endDateTime) {
                    const duration =
                      endDateTime.getTime() -
                      (startDateTime?.getTime() || date.getTime());
                    if (duration > 0) {
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                ...
                inputProps={{
                  min: startDateTime
                    ? format(startDateTime, "yyyy-MM-dd'T'HH:mm")
                    : undefined,
                }}
                InputLabelProps={{ shrink: true }}
              />
