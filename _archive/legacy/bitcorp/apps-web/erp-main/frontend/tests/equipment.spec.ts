...
test.describe('Equipment Management with SWR', () => {
  ...
  test('should load and display equipment list using SWR', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="status-available"]')
    ).toBeVisible();
  ...
  test('should handle equipment search functionality', async ({ page }) => {
    ...
    await expect(page.locator('text=CAT 320 Excavator')).toBeVisible();
    await expect(
      page.locator('text=John Deere 850K Bulldozer')
    ).not.toBeVisible();
  });
  ...
  test('should handle equipment status filtering', async ({ page }) => {
    ...
    await page.route('**/api/v1/equipment?status=available*', async (route) => {
      const filteredResponse = {
        ...equipmentListResponse,
        equipment: equipmentListResponse.equipment.filter(
  ...
  test('should handle equipment creation with SWR mutation', async ({
    page,
  }) => {
    ...
    // Fill equipment form
    await page
      .locator('[data-testid="equipment-name"]')
      .fill('New Test Equipment');
    ...
    await expect(
      page.locator('text=Equipment created successfully')
    ).toBeVisible();
  });

  test('should handle equipment status update with optimistic updates', async ({
    page,
  }) => {
    ...
    await page
      .locator(
        '[data-testid="equipment-item"]:first-child [data-testid="status-button"]'
      )
      .click();
    ...
    await expect(
      page.locator('[data-testid="status-maintenance"]')
    ).toBeVisible();
  });

  test('should display equipment statistics using SWR stats hook', async ({
    page,
  }) => {
    ...
    await expect(page.locator('[data-testid="total-equipment"]')).toContainText(
      '2'
    );
    await expect(page.locator('[data-testid="available-count"]')).toContainText(
      '1'
    );
    await expect(page.locator('[data-testid="in-use-count"]')).toContainText(
      '1'
    );
  ...
  test('should handle real-time data updates with SWR revalidation', async ({
    page,
  }) => {
    // Initial load
    await expect(page.locator('[data-testid="equipment-list"]')).toBeVisible();
