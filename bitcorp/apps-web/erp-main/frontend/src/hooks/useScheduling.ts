...
export function useScheduleList(filters?: ScheduleFilters) {
  ...
  if (filters?.equipment_id)
    params.append('equipment_id', filters.equipment_id.toString());
...
export function useUpdateSchedule() {
  const { trigger, isMutating, error } = useSWRMutation(
    ...
    async (
      url: string,
      { arg }: { arg: { id: number; data: EquipmentScheduleUpdateRequest } }
    ) => {
...
export function useScheduleStats(
  filters?: Omit<ScheduleFilters, 'page' | 'per_page'>
) {
  const params = new URLSearchParams();
  ...
  const url = `/api/v1/scheduling/equipment/${
