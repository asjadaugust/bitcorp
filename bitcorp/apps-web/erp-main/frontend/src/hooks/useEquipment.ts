...
export const equipmentKeys = {
  ...
  utilization: (id: number) =>
    [...equipmentKeys.all, 'utilization', id] as const,
  ...
  const { trigger, isMutating, error } = useSWRMutation(
      ...
      mutationFetcher<Equipment>(url, { method: 'POST', data: arg }),
    {
      onSuccess: () => {
        ...
        mutate(
          (key) => typeof key === 'string' && key.startsWith('/equipment')
        );
      },
    }
  );
...
export const useUpdateEquipment = () => {
  const { trigger, isMutating, error } = useSWRMutation(
    ...
    async (
      url: string,
      { arg }: { arg: { id: number; data: EquipmentUpdateRequest } }
    ) =>
      mutationFetcher<Equipment>(`/equipment/${arg.id}`, {
        method: 'PUT',
        data: arg.data,
      }),
    {
...
export const useUpdateEquipmentStatus = () => {
  const { trigger, isMutating, error } = useSWRMutation(
    ...
    async (
      url: string,
      { arg }: { arg: { id: number; status: EquipmentStatus } }
    ) =>
