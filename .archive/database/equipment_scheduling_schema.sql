...
DECLARE
    current_time TIMESTAMP WITH TIME ZONE := p_start_date;
...
BEGIN
    ...
    LOOP
        ...
        IF current_time < schedule_record.start_datetime THEN
            RETURN QUERY SELECT 
                current_time,
                schedule_record.start_datetime,
                ROUND(EXTRACT(EPOCH FROM (schedule_record.start_datetime - current_time)) / 3600.0, 2),
                'available'::VARCHAR(20);
        END IF;
        
        -- Mark the scheduled time as occupied
        RETURN QUERY SELECT 
            GREATEST(current_time, schedule_record.start_datetime),
            schedule_record.end_datetime,
            ROUND(EXTRACT(EPOCH FROM (
                schedule_record.end_datetime - GREATEST(current_time, schedule_record.start_datetime)
            )) / 3600.0, 2),
            'scheduled'::VARCHAR(20);
        
        -- Move current_time past this schedule
        current_time := GREATEST(current_time, schedule_record.end_datetime);
    END LOOP;
    
    -- Check for availability after the last schedule
    IF current_time < p_end_date THEN
        RETURN QUERY SELECT 
            current_time,
            p_end_date,
            ROUND(EXTRACT(EPOCH FROM (p_end_date - current_time)) / 3600.0, 2),
            'available'::VARCHAR(20);
    END IF;
END;
$$ LANGUAGE plpgsql;
